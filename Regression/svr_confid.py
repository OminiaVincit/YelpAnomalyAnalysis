  ...
  for site in ['yelp', 'tripadvisor']:
    for index in range(50):
      ...
      #for ftype in ['STR', 'TOPICS_64', 'tfidf', 'LIWC', 'INQUIRER', 'GALC']:
        ...
        msg = 'categ=%s, ftype=%s, index=%d, rmse=%.10f, rho=%.10f, pval=%.10f, mse=%.10f, mae=%.10f, pre_logfit=%.10f, base_logfit=%.10f' %(site, ftype, index, rmse, rho, pval, mse, mae, pre_logfit, base_logfit)
        print msg
        logging.info(msg)
      dat = np.vstack(dat)
      t_predict = eval_conf(dat)
      mae, mse, rmse, rho, pval, pre_logfit = score(t_predict, y_test, h_test, N_test)
      msg = 'categ=%s, ftype=ConfAll, index=%d, rmse=%.10f, rho=%.10f, pval=%.10f, mse=%.10f, mae=%.10f, pre_logfit=%.10f, base_logfit=%.10f' %(site, index, rmse, rho, pval, mse, mae, pre_logfit, base_logfit)
      print msg
      logging.info(msg)
